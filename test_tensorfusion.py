...
from Architectures.architectures import TENSORFUSION_ARCHITECTURES
...
try:
    TENSORFUSION_ARCHITECTURES[model_name]
except KeyError:
    available_names = ", ".join(TENSORFUSION_ARCHITECTURES.keys())
...
train_mlp(model, learning_rate, train_dataloader, 60, loss_function, optimizer, 'tensorfusion', test_dataloader)
if learning_rate != 0:
    base_name = f'model_{model_name}_lr_{str(learning_rate).replace(".", "")}'
else:
    base_name = f'model_{model_name}_adam'

results_path = join('Results', 'tensorfusion')
