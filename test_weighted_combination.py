...
from Models.DeepFusion import WeightedCombinationClassifier
...
train_mlp(model, learning_rate, train_dataloader, 200, loss_function, optimizer, 'weighted_combination', test_dataloader)
if learning_rate != 0:
    base_name = f'model_{model_name}_lr_{str(learning_rate).replace(".", "")}'
else:
    base_name = f'model_{model_name}_adam'

results_path = join('Results', 'weighted_combination')
